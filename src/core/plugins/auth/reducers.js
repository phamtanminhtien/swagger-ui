...
export default {
  ...
  [AUTHORIZE]: (state, { payload }) => {
    ...
    securities.entrySeq().forEach(([key, security]) => {
      ...
      const scheme = security.getIn(["schema", "scheme"])

      if (type === "http" && scheme === "bearer") {
        const tokenManager = security.get("value")
        if (tokenManager) {
          const selectedTokenName = tokenManager.get("selectedToken")
          const tokens = tokenManager.get("tokens") || fromJS([])
          const selectedToken = tokens.find(t => t.get("name") === selectedTokenName)
          const tokenValue = selectedToken ? selectedToken.get("value") : ""

          const newSecurity = security
            .set("value", tokenValue)
            .set("tokenManager", tokenManager)

          map = map.set(key, newSecurity)
        } else {
          map = map.set(key, security)
        }
      } else if (type === "apiKey" || type === "http") {
        map = map.set(key, security)
      } else if (type === "basic") {
